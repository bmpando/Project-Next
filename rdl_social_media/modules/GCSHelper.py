...
class GCSHelper:
    ...
    def __init__(self, file_name, from_bucket, to_bucket, new_file_name, sub_folder):
        ...
        if str(self.sub_folder) is not None:
            #self.new_blob_name = str(self.sub_folder) + str(self.new_file_name)
            self.new_blob_name = str(self.new_file_name)
        else:
            self.new_blob_name = str(self.new_file_name)
    ...
    def move_file(self):
        try:
            ...
            # copy file
            new_blob = source_bucket.copy_blob(source_blob, destination_bucket, self.new_blob_name)
            # delete file
            source_blob.delete()
            logging.debug('File {} Successfully Moved '.format(source_blob))
        except NotFound as n:
            logging.info('File already moved')
            return
